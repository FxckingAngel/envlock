...
This project was built by Korone. AI assistance (Claude) was used for planning, architecture specs, command design, debugging discussion, and the explanatory comments in the source code — but the implementation itself (source logic, tests, and fixes) was written and run by the author, not generated by the AI in this repository. Every command was built, tested, and verified against real output before being committed. Design tradeoffs (e.g. identity-key-on-disk vs. OS keychain, the in-memory `run` decision, and the CI bridge design) were made by the author, and debugging of real issues (including two secrets-handling incidents during development, both caught and fixed) was done by the author working through actual failures.
